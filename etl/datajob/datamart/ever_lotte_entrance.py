...
class LotteEverEntrance:
    ...
    def save(cls):
        ...
        # 에버랜드 입장객 정보
        df_ever = cls.__get_ever_df(everland_num, df_ent)

        # 날짜(월별)만 가져오기
        dates = [row['STD_DATE'] for row in df_ever.collect()]

        # 롯데월드 입장객 정보
        df_lotte = cls.__get_lotte_df(lotteworld_num, df_ent, dates)
    ...
    @classmethod
    def __get_lotte_df(cls, lotteworld_num, df_ent, dates):
    ...
    @classmethod
    def __get_ever_df(cls, everland_num, df_ent):
