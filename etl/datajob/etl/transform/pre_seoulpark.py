...
class PreSeoulParkTransformer:
    @classmethod
    def transform(cls):
        seoulpark_num = cls.__get_theme_num()

        # 입장객수 데이터 가져오기
        df_ent = cls.__get_entrance(seoulpark_num)

        # 날씨 데이터 가져오기
        df_weather = cls.__get_weather(seoulpark_num)

        # 미세먼지 데이터 가져오기
        df_air = cls.__get_air(seoulpark_num)

        # 지하철 승하차수 데이터 가져오기
        df_subway = cls.__get_subway(seoulpark_num)

        # 이벤트 데이터 가져오기
        df_event = cls.__get_event(seoulpark_num)

        # 네비게이션 데이터 가져오기
        df_nav = cls.__get_navigation(seoulpark_num)
        ...
        # DM에 저장
        overwrite_trunc_data(DataMart, df_fin, 'PRE_SEOULPARK')
    ...
    @classmethod
    def __get_navigation(cls, seoulpark_num):
    ...
    @classmethod
    def __get_event(cls, seoulpark_num):
    ...
    @classmethod
    def __get_subway(cls, seoulpark_num):
    ...
    @classmethod
    def __get_air(cls, seoulpark_num):
    ...
    @classmethod
    def __get_weather(cls, seoulpark_num):
    ...
    @classmethod
    def __get_entrance(cls, seoulpark_num):
